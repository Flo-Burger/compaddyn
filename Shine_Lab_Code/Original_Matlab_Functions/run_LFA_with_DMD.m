function [lmse,msd,e_vecs,lambdas] = run_LFA_with_DMD_1(data_ts,n_lag, exp_var_lim,delta_t)
...
%
    ...
    for subj = 1: n_subjs
        ...
        % Calculate DMD modes
        [e_vec, e_val_full] = eig(A_tilde);
        e_val = diag(e_val_full);
        lambda = log(e_val)/delta_t;
    
        %e_vals{subj} = e_val; % Eigenvalues
        lambdas{subj} = lambda; 
        e_vecs{subj} = Y*V*inv(S)*e_vec; % Eigenvectors
...
n_lag = 15;           % how many future time points to predict
exp_var_lim = 95;     % retain components explaining 95% of variance
delta_t = 0.5;        % time interval between samples (in seconds, for example)
subj = 1;

[lmse, msd, e_vecs, lambdas] = run_LFA_with_DMD_1(fmri_data, n_lag, exp_var_lim, delta_t);

% === LMSE ===
disp('=== LMSE ===');
disp(['MATLAB: ', num2str(squeeze(lmse(1,:,subj)), ' %.4f')]);

% === MSD ===
disp(' ');
disp('=== MSD ===');
disp(['MATLAB: ', num2str(squeeze(msd(1,:,subj)), ' %.4f')]);


% Sort by modulus of discrete-time eigenvalues (before log)
eigvals = exp(delta_t * lambdas{subj}); % convert λ back to discrete-time eigenvalues
[~, sort_idx] = sort(abs(eigvals), 'descend');

lambda_sorted = lambdas{subj}(sort_idx); % now sorted by |eigenvalue|

% Display
disp('=== DMD Eigenvalues (λ) sorted by |eigenvalue| ===');
for i = 1:10
    lambda = lambda_sorted(i);
